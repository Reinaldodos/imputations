...
## Chemins
path_ech <- "P:/stat/01_etudes-methodes/@commun/EMEBI/échantillon/échantillon 202201/datas"
Path_Data <- file.path("../data")
Path_Prod <- file.path(paste0(
  "../production_",
  year(ymd(mstat)),
...
fileC_ventil_intro <- list.files(Path_C,
  pattern = "ventil_intro"
)[1]

fileC_ventil_exped <- list.files(Path_C,
  pattern = "ventil_exped"
)[1]
...
imput_intro <- read.csv2(
  file = file.path(Path_C, fileC_imp_intro),
  sep = ";",
...
imput_exped21 <- read.csv2(
  file = file.path(Path_C, fileC_imp_exped21),
  sep = ";",
  colClasses = c(siren = "character")
) %>%
  select(siren, period, prediction, method, method_ref)
...
imput_exped29 <- read.csv2(
  file = file.path(Path_C, fileC_imp_exped29),
  sep = ";",
  colClasses = c(siren = "character")
) %>%
  select(siren, period, prediction, method, method_ref)
...
ventil_intro <- read.csv2(
  file = file.path(Path_C, fileC_ventil_intro),
  sep = ";", na.strings = TRUE,
  colClasses = c(
    siren = "character",
    nc8 = "character"
...
ventil_exped <- read.csv2(
  file = file.path(Path_C, fileC_ventil_exped),
  sep = ";", na.strings = TRUE,
  colClasses = c(
    siren = "character",
    nc8 = "character"
...
ech_i <- echantillon %>%
  filter(
    type.d.enquête %in% c("intro", "intro+exped"),
...
ech_e <- echantillon %>%
  filter(
    type.d.enquête %in% c("expéd", "intro+exped"),
...
#+++++++++++++++++++++++++
saveWorkbook(wb, file.path(Path_C, paste0(
  "Controles_Chiffre_",
  mstat, ".xlsx"
)))
