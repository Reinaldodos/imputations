...
### Date statistique Ã  mettre Ã  jour ###
########################################
# mstat <- "2022-08-01"
...
Path_PC <- "../production_202307/output_PC"
...
### Fonctions ###
#################
source(file = "../programs/fonctions_controles.R", encoding = "UTF-8")

### Lecture des imputations ###
###############################

## Introductions
imput_intro <- read.csv2(file = file.path(Path_PC, filePC_imp_intro),
                         sep = ";", 
...
imput_exped29 <- read.csv2(file = file.path(Path_PC, filePC_imp_exped29),
                           sep = ";", 
                           colClasses = c(siren="character")) %>% 
  select(siren, period, prediction, method, method_ref)
...
## Introductions
ventil_intro <- read.csv2(file = file.path(Path_PC, filePC_ventil_intro),
                          sep = ";", 
                          colClasses = c(siren = "character",
                                         nc8 = "character"), 
                          na.strings = "") %>% 
...
list_i_siren10M <- list_i_siren10M %>% 
  ...
  left_join(mutate(sample_intro, year = year(date_beg)), 
            by = c('siren', 'year')) %>%
  select(-year)
...
#+++++++++++++++++++++++++
saveWorkbook(wb, file.path(Path_PC, paste0("Controles_Pré-chiffre", 
                                           mstat, ".xlsx")))
