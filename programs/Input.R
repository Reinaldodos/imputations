...
Classes <- c(siren = "character", sire = "character", SIRE = "character",
             siret = "character",
...
data_treatment <- function(source_file, file, rename_list, total_variable, ...){
  condition <- sprintf("(period >= '%s') & (period <= '%s')",
  ...
  towards_data <- import_input(source_file = source_file,
                               file = file) %>%
...
setMethod(
  ...
  definition = function(object, flow){
    ...
    variable <- na.omit(c("siren_new", "period",
    ...
    if (!file.exists(file.path(object@input_directory, filename))){
      ...
      if (object@use_historical_basis){
        ...
        # Read current file
        source_file <- slot(object = object,
      ...
      imput_data <- left_join(
        imput_data, delete_data, by = 'siren'
      ) %>%
        rowwise() %>%
      ...
      sirens_extra <- subset(sample, subset = (siren %notin% imput_data$siren), 
                             select = siren) %>%
        unique() %>%
        flatten_chr()
      if (flow == "I"){
        ...
        imput_data <- bind_rows(
          imput_data,
          data.frame(siren = rep(sirens_extra, 2),
                     period = object@date_ref,
    ...
    if (object@save_historical_input){
      saveRDS(imput_data,
              file.path(object@historical_directory,
                        filename))
    }
    return(imput_data)
  }
...
setMethod(
  ...
  definition = function(object, flow, condition){
    ...
    if (!file.exists(file.path(object@input_directory, filename))){
      delete_data <- import_delete(object = object)
      source_file <- slot(object,
                          sprintf("%s_ventil", flow_name))
      detail_data <- source_file$files %>%
        map_df(
          ~data_treatment(source_file = source_file,
                          file = .x,
                          rename_list = c('siren' = 'sire'),
                          total_variable = "sire",
                          condition)
      ...
      modified_data <- inner_join(
        ...
        mutate(siren_new = ifelse(test = is.na(siren_repreneur),
               ...
               vart_new = ifelse(test = is.na(ratio),
                                 yes = vart,
                                 no = vart*ratio)) %>%
        group_by(siren_new, period, a129, nc8, payp, pyod,
...
setMethod(
  ...
  definition = function(object, flow, date){
    flow_dict <- data.frame(flow = c('E', 'I'),
                            flow_name = c('exped', 'intro'))
    historical_basis <- readRDS(file.path(object@historical_directory,
                                          "Base_historique.rds")) %>%
      ...
      subset(select = c(siren, period, prediction, period_last, a129, nc8,
...
setGeneric(
  name = "import_gazelec",
  def = function(object, flow, pass_names){standardGeneric("import_gazelec")}
)
setMethod(
  f = "import_gazelec",
  signature = "Input",
  definition = function(object, flow, pass_names){
    ...
    if (object@use_gazelec_file){
      pass_table <- import_pass_table(object, pass_names) %>%
        subset(subset = (year == year(object@date_ref)),
               select = c(nc8, a129))
      gazelec_data <- import_input(object@gazelec, object@gazelec$files) %>%
        subset(subset = !is.na(mois)) %>%
        mutate(period = make_date(year = an, month = mois, day = 1),
               ...
               nc8 = str_pad(string = as.character(nc8),
                             width = 8, side = "left", pad = "0")) %>%
        group_by_at(group) %>%
        mutate(prediction = sum(dist_prediction)) %>%
        ungroup() %>%
        subset(subset = (flux == flow),
               select = c(siren, period, prediction, method, method_ref,
...
setMethod(
  ...
  definition = function(object, pass_names){
    pass_data <- object@pass$files %>%
      map_df(~get_pass_table(pass_file = object@pass,
                             file = .x,
