...
compute_coverage <- function(response_data, estimation, 
                             NR_list, output_dates, f, 
                             filename){
  workbook <- createWorkbook()
  v <- names(estimation)[1]
  for (date in output_dates){
    sheet <- createSheet(workbook, 
                         sheetName = sprintf("%s %s", 
                                             toupper(v), 
                                             format(as_date(date), 
                                                    "%Y-%m-%d")))
    output_obs <- response_data %>%
      ...
      rowwise() %>%
      mutate(
        `Vol.(litre) - EMEBI + DAU` = sum(
          `Vol.(litre) - EMEBI`, `Vol.(litre) - DAU`, na.rm = T
        ),
        `Val.(euros) - EMEBI + DAU` = sum(
          `Val.(euros) - EMEBI`, `Val.(euros) - DAU`, na.rm = T
        ),
        `*R - EMEBI + DAU` = sum(
          `*R - EMEBI`, `*R - DAU`, na.rm = T
        )
      ) %>%
      select(
        as.name(v), 
    ...
    output_imput <- subset(estimation,
                           subset = ((period == as_date(date)) &
                                       (flow == f))) %>%
      data.table::setnames(old = c('vart_NR_prediction', 'usup_NR_prediction', 'nb_NR'),
    ...
    output_TC <- response_data %>%
      ...
      pivot_wider(names_from = type_enquete, 
                  ...
                  values_fill = 0) %>%
      rowwise() %>%
      mutate(vart_EMEBI_DAU = sum(vart_EMEBI, vart_DAU, na.rm = T),
             usup_EMEBI_DAU = sum(usup_EMEBI, usup_DAU, na.rm = T))
    response <- response_data %>%
      subset(subset = ((period == (as_date(date) - years(1))) & 
                         (flow == f))) %>%
      anti_join(
        NR_list[(NR_list$period == as_date(date))
                & (NR_list$flow == f),], 
