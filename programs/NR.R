pacman::p_load(tidyverse,lubridate,doParallel,rlang,zoo,multidplyr,data.table,tsbox,RJDemetra)
`%notin%` <- Negate(`%in%`)
...
NR <- setClass("NR", 
         ...
         slots = c(endogenous = "tbl_null", exogenous = "tbl_null",
                   endo_name = "character", exog_name = "character_null",
                   ER = "tbl_null", ER_name = "character_null",
         ...
         validity = function(object){
           ...
           if ((object@flow != 'I')
               & (object@flow != 'E')){
             return("Le param?tre 'flow' prend seulement deux options : 'I' pour introduction et 'E' pour exp?dition.")
           }
           
           # Vérification de régime pour l'introduction
           if ((object@flow == "I") & (object@reg_exped != 0)){
             stop("Pour les introductions, le régime doit être '0'.")
           }
...
setMethod(f = "taking_exog",
          ...
          definition = function(object, prediction_period, siren){
            ...
            prediction <- (prediction %>% left_join(
              data[,c('siren', 'period', object@exog_name)], 
              by=c('siren', 'period'),
              )
...
setGeneric(name = "taking_ER",
           def = function(object, prediction_period, siren){
             standardGeneric("taking_ER")
           })
setMethod(f = "taking_ER",
          signature = "NR",
          definition = function(object, prediction_period, siren){
            ...
            prediction <- (prediction %>% left_join(
              data[,c('siren', 'period', object@ER_name)], 
              by=c('siren', 'period'),
            )
...
setGeneric(name = "taking_mean",
           def = function(object, prediction_period, siren){
             standardGeneric("taking_mean")
           })
setMethod(f = "taking_mean",
          signature = "NR",
          definition = function(object, prediction_period, siren){
            # print(full_join(
            #   data.frame(siren = rep(siren, each = length(prediction_period)),
            ...
            %>% group_by(siren)
            %>% arrange(period)
            %>% mutate(prediction = lag(cummean.na(UQ(sym(object@endo_name)))),
                       method = "taking_mean")
...
setGeneric(name = "taking_last_year",
           def = function(object, prediction_period, siren,
                          alternative = TRUE){
             standardGeneric("taking_last_year")
           })
setMethod(f = "taking_last_year",
          signature = "NR",
          definition = function(object, prediction_period, siren, 
                                alternative = TRUE){
            prediction <- (data.frame(siren = rep(siren, each=length(prediction_period)),
                                      period = as.Date(rep(prediction_period, length(siren))))
            ...
            if (alternative){
              new_prediction <- taking_mean(object = object, 
              ...
              if (nrow(new_prediction)>0){
                prediction <- bind_rows(
                  ...
                  y = anti_join(prediction, new_prediction[c('siren', 'period')], by=c('siren', 'period'))
                )}
            }
            return(as_tibble(prediction[,c('siren', 'period', 'prediction', 'method')]))
          })
...
setGeneric(name = "launch_sarima",
           def = function(object, prediction_period, 
                          learning_first, siren, nbproc=10,
                          alternative = TRUE){
             standardGeneric("launch_sarima")
           })
setMethod(f = "launch_sarima",
          signature = "NR",
          definition = function(object, prediction_period, 
                                learning_first, siren, nbproc=10, 
                                alternative = TRUE){
            ...
            start_time <- Sys.time()
            prediction <- foreach(siren = siren, .combine=rbind,
                                  .packages = c('dplyr', 'tsbox', 
                                                'RJDemetra', 'lubridate')) %dopar%{
              data <- (data.frame(
                period = seq.Date(
                  from = learning_first,
              ...
              if (!inherits(model, 'try-error')){
                if(!is.null(model$forecast)){
                  pred <- (model$forecast[,'fcst']
                           ...
                           %>% left_join(
                             data.frame(siren = rep(siren, length(pred_period)),
                                        period = pred_period),.,
                             by = 'period'
                           ))
                }else{pred <- data.frame(siren = rep(siren, length(pred_period)),
                                         period = pred_period,
                                         prediction = NA)}
              }else{pred <- data.frame(siren = rep(siren, length(pred_period)),
                                       period = pred_period,
            ...
            prediction <- mutate(prediction, method = "launch_sarima")
            if (alternative){
              new_prediction <- taking_last_year(object = object, 
                                                 prediction_period = prediction_period,
...
setMethod(f = "create_ts",
          ...
          definition = function(object, data,
            ...
            colnames(ttmp) <- append(c(object@endo_name, object@exog_name), months)
            ttmp[is.na(ttmp)] <- 0
            return(ttmp)
          })


setGeneric(name = "launch_reglin",
           def = function(object, prediction_period, siren,
                          nb_learning_year = 5, nbproc=10, 
                          alternative = TRUE){
             standardGeneric("launch_reglin")
           })
setMethod(f = "launch_reglin",
          signature = "NR",
          definition = function(object, prediction_period, siren, 
                                nb_learning_year = 5, nbproc=10, 
                                alternative = TRUE){
			pred_period <- seq(from = min(prediction_period),
            ...
            prediction <- foreach(siren = siren, .combine = rbind, 
              ...
              data <- (object@endogenous 
                       ...
                       %>% mutate(
                         ...
                         july = case_when(
                           ((month(period) == 7) 
                            & ((UQ(sym(object@exog_name)) == 0 
                                | is.na(UQ(sym(object@exog_name)))))) ~ 1,
                           TRUE ~ 0
                         ),
                         august1 = case_when(
                           ((month(period) == 8)
                            & ((UQ(sym(object@exog_name))[month(period) == 7] == 0)
                               | is.na(UQ(sym(object@exog_name))[month(period) == 7]))) ~ 1,
                           TRUE ~ 0
                         ),
                         august2 = case_when(
                           ((month(period) == 8)
                         ...
                         september = case_when(
                           ((month(period) == 9)
                            & ((UQ(sym(object@exog_name))[month(period) == 8] == 0)
                               | is.na(UQ(sym(object@exog_name))[month(period) == 8]))) ~ 1,
                           TRUE ~ 0
                         )))
              # if (object@outlier_treatment){
              #   data <- subset(data, 
              #                  (period < object@outlier_first)
              #                  | (period > object@outlier_last))
              # }
              print(data)
              data <- create_ts(object, data)
              model <- try(lm(as.formula(paste(object@endo_name, 
                                               paste(c(object@exog_name, 
                                                       'july', 'august1', 
                                                       'august2', 'september'),
                                                     collapse = '+'),
                                               sep = '~')),
                              data = window(data, end = c(year(min(pred_period)-months(1)),
                                                          month(min(pred_period)-months(1))))),
                           silent = TRUE)
              if (!inherits(model, 'try-error')){
                ...
                if (!inherits(model, 'try-error')){
                  pred <- data.frame(
                    ...
                    period = pred_period,
                    prediction = predict(model, newdata = window(
                      data, start = c(year(min(pred_period)),
            ...
            stopCluster(cl)
            time_delta <- Sys.time() - start_time
            prediction <- mutate(prediction, method = "launch_reglin")
...
setGeneric(name = "estimNR",
           def = function(object, prediction_period, siren,
                          ref_period, learning_first,
                          nb_learning_year = 5, nbproc=10, alternative = TRUE,
                          na_imput = TRUE,  negative_imput = TRUE, 
...
setMethod(f = "estimNR",
          ...
          definition = function(object, prediction_period, siren,
            ...
            if (object@flow == 'E' & object@reg_exped == 29){
              group_siren <- (object@endogenous[object@endogenous$siren %in% siren,]
                              %>% group_by(siren)
                              %>% summarise(nobs = sum((period < (ymd(min(prediction_period)) - years(1)))
                                                       & (UQ(sym(object@endo_name)) > 0)))
              ...
              for (method_ref in unique(group_siren$method_ref)){
                siren_list <- group_siren[group_siren$method_ref == method_ref,]$siren
                if (method_ref == "taking_last_year"){
                  pred <- do.call(get(method_ref), args = list(object, prediction_period, siren_list, alternative))
                  # if (na_imput){pred[is.na(pred$prediction),]$prediction <- 0}
                }else{
                  pred <- do.call(get(method_ref), args = list(object, prediction_period, learning_first,
                                                           siren_list, nbproc, alternative))
                }                    
                prediction <- rbind(prediction, pred)
              }
            }else{                
              if ((ref_period == prediction_period) | (object@flow == "E" & object@reg_exped == 21)){
                ...
                # write.csv2(group_siren, "group_siren.csv", row.names = FALSE)
                if (object@flow == "E" & object@reg_exped == 21){
                  group_siren[group_siren$siren %in% 
                                object@ER[object@ER$period %in% 
                                            prediction_period,]$siren,]$method_ref <- "taking_ER"
                }
              }else{
                  group_siren <- (left_join(object@endogenous[object@endogenous$siren %in% siren,], 
                                            object@exogenous[object@exogenous$siren %in% siren,],
                                            by=c('siren', 'period'))
                                  %>% group_by(siren) %>% summarise(
                                    notna_exog = sum(
                                      (is.na(UQ(sym(object@exog_name))) == FALSE)
                                    ),                                  
                                    nobs = sum(period < (ymd(min(prediction_period)) - years(1)))
                                  )
                                  %>% mutate(method_ref = case_when(
                                    ((nobs == 0) & (notna_exog > 0)) ~ 'taking_exog',
                                    ((nobs == 0) & (notna_exog == 0)) ~ 'taking_mean',
                                    ((nobs < 12*nb_learning_year) & (notna_exog == 0)) ~ 'taking_last_year',
                                    ((nobs >= 12*nb_learning_year) & (notna_exog == 0)) ~ 'launch_sarima',
                                    TRUE ~ 'launch_reglin'
                                  )))
              }
              # print(group_siren)
              ...
              for (method_ref in unique(group_siren$method_ref)){
                ...
                if (method_ref %in% c('taking_exog', 'taking_mean', 'taking_ER')){
                  pred <- do.call(get(method_ref), args = list(object, prediction_period, siren_list))
                }else{
                  if (method_ref == 'taking_last_year'){
                    pred <- do.call(get(method_ref), args = list(object, prediction_period, siren_list, alternative))
                    }else{
                      if (method_ref == 'launch_sarima'){
                        pred <- do.call(get(method_ref), args = list(object, prediction_period, learning_first, siren_list, nbproc, alternative))
                        }else{
                          pred <- do.call(get(method_ref), args = list(object, prediction_period, siren_list, nb_learning_year, nbproc, alternative))
                        }
                    }
                  }
                prediction <- rbind(prediction, pred)
            ...
            # }
          if (na_imput){prediction[is.na(prediction$prediction),]$prediction <- 0}
          if (negative_imput){prediction[prediction$prediction < 0,]$prediction <- 0}
          if (get_method_ref){return(left_join(prediction, group_siren[,c('siren', 'method_ref')],
                                           by=c('siren')))}else{return(prediction)}
        })
...
get_NR_list <- function(sample, endogenous, date){
  ...
  sample <- subset(sample, subset = (date_beg == date_sample), 
                   select = siren) %>%
    unique()
...
setGeneric(
  ...
  def = function(object, date, sample, 
                 input, learning_from, nb_years_regressions,
                 nbproc = 5){
...
setMethod(
  f = "launch_estimation", 
  signature = "NR", 
  definition = function(object, date, sample, 
                        input, learning_from, nb_years_regressions,
                        nbproc = 5){
    ...
    imput_data <- estimNR(object = object, 
    ...
    saveRDS(imput_data, 
            file.path(input@output_directory, 
                      sprintf("estim_%s_%s_ref%s.rds", 
                              flow_name, 
                              format(as_date(date), "%Y%m"), 
                              format(date_ref, "%Y%m"))))
    return(imput_data)
...
setGeneric(
  ...
  def = function(object, dates, sample, input, learning_from, 
                 nb_years_regressions = 5, nbproc = 5){
...
setMethod(
  ...
  signature = "NR",
  definition = function(object, dates, sample, input, learning_from, 
                 nb_years_regressions = 5, nbproc = 5){
    flow_name <- get_flow_name(object)
    ...
    if (file.exists(filename)){
      ...
      imput_data <- dates %>%
        map_df(~launch_estimation(object = object, date = .x, sample = sample,
                                  input = input, learning_from = learning_from, 
...
setGeneric(name = "distribution",
           def = function(object, estim_result, 
                          data_distribution, sirens="all",
                          ventil_imput = 'all', ventil_output = NULL,
                          nbproc = 10, lag_width = 1){
             standardGeneric("distribution")
           })
setMethod(f = "distribution",
          signature = "NR",
          definition = function(object, estim_result, 
                                data_distribution, sirens = "all",
                                ventil_imput = 'all', ventil_output = NULL,
                                nbproc = 10, lag_width = 1){
            if ((ventil_imput != 'all') & (class(ventil_imput) == 'character')){
            ...
            distribution <- (data_distribution[data_distribution$siren %in% selected_sirens,]
            ...
            dist <- (distribution
                     ...
                     %>% group_by(siren)
                     %>% mutate(period_last = zoo :: rollapply(
                       data.table :: shift(period, 1, fill = NA, type = 'lag'),
                       width = 1, FUN = copy, align = 'right', fill = NA
                     ))
                     %>% ungroup())
            dist_prediction <- (left_join(estim_result[estim_result$siren %in% selected_sirens, ], 
                                         dist,
                                         by = c('siren', 'period')))
            dist_prediction <- (dist_prediction
                                ...
                                %>% mutate(dist_prediction = prediction*ratio))
...
setGeneric(
  ...
  def = function(object, input, imput_result, date,
                 detail_data){
...
setMethod(
  ...
  signature = "NR", 
  definition = function(object, input, imput_result, date,
                        detail_data){
    result <- filter(.data = imput_result, period == as_date(date))
    distribution_result <- distribution(
      object = object, 
      estim_result = result, 
      data_distribution = detail_data[,c('siren', 'period', 'a129', 'nc8', 'pyod', 
                                         'payp', 'dept', 'temo', 'natr', 'regdem', 
                                         'conf', object@endo_name)], 
...
setGeneric(
  ...
  def = function(object, input, imput_result, dates, detail_data){
...
setMethod(
  ...
  signature = "NR", 
  definition = function(object, input, imput_result, dates, detail_data){
    ...
    if (file.exists(filename)){
      ...
      distribution_data <- dates %>%
        map_df(~launch_distribution(
          object = object, input = input, imput_result = imput_result, 
          date = .x, detail_data = detail_data
        )) %>%
        bind_rows()
...
setMethod(
  ...
  definition = function(object, input, current_result, type = "ventil"){
    ...
    if (file.exists(filename)){
      ...
      past_month <- setdiff(input@date_publication, input@date_prediction) %>% 
        as_date()
      past_simulation <- past_month %>%
        import_historical_simulation(
          object = input, 
          flow = object@flow, 
          date = .
        )
      past_NR <- past_month %>%
        ...
        bind_rows()
      past_simulation <- semi_join(past_simulation, past_NR, by = c('siren', 'period'))
      if (type == "ventil"){
        result <- bind_rows(arrange(past_simulation, period), current_result)
      }else{
        result <- bind_rows(
          past_simulation %>% 
...
setGeneric(
  name = "add_gazelec", 
  def = function(object, input, result, type, by_regdem = F,
                 siren_list = c('095580841', '440117620', '444619258'), 
                 pass_names){standardGeneric("add_gazelec")}
)
setMethod(
  f = "add_gazelec", 
  signature = "NR", 
  definition = function(object, input, result, type, by_regdem = F,
                        siren_list = c('095580841', '440117620', '444619258'), 
                        pass_names){
    gazelec_data <- import_gazelec(input, object@flow, pass_names = pass_names)
    flow_name <- get_flow_name_register(object)
    if (by_regdem){
      filename <- file.path(input@output_directory, 
                            sprintf("%s_%d_%s_with_gazelec.rds", 
...
setGeneric(
  name = "remove_msd", 
  def = function(object, msd, result, type, by_regdem = F, input){
...
setMethod(
  f = "remove_msd", 
  signature = "NR", 
  definition = function(object, msd, result, type, by_regdem = F, input){
...
export_to_historical_basis <- function(input, data, flow, save = T){
  exo <- ifelse(test = grepl(pattern = "_PC", x = input@output_directory), 
                   yes = "prechiffre", 
                   no = "chiffre")
  historical_basis <- readRDS(file.path(input@historical_directory, 
                                        "Base_historique.rds")) %>%
    mutate(period_last = as.Date(period_last), 
  ...
  NR_list <- bind_rows(
    ...
    mutate(
      ...
      last_trimestre = sprintf("%d.%d", year(last_period), 
                               quarter(last_period)), 
      imex = ifelse(test = (flow == "I"), yes = 3, no = 4)
    )
  return(NR_list)
}